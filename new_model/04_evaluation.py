"""
=============================================================================
Stage 4: Comprehensive Model Evaluation Script
=============================================================================
...
def run_evaluation():
    print("="*80)
    print("  CROP YIELD PREDICTION — COMPREHENSIVE EVALUATION")
    ...
    # Inverse-scale predictions back to original Kg per ha scale
    unscaled_preds = {}
    print("\n[1/3] Inverse scaling predictions back to Kg per ha units...")
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write("="*80 + "\n")
        f.write("  CROP YIELD PREDICTION — EXPERIMENT COMPARISON REPORT (ICRISAT)\n")
        f.write("="*80 + "\n\n")
        f.write(f"Test Set Size: {len(y_test_raw):,} samples (2014 - 2017)\n\n")
        ...
        f.write(f"  RMSE: {best_rmse:.2f} Kg/ha\n")
        ...
        for name, pred in unscaled_preds.items():
            ...
            f.write(f"\n  [{name}]\n")
            f.write(f"    RMSE (Error):       {np.sqrt(np.mean(residuals**2)):.2f} Kg/ha\n")
            f.write(f"    MAE (Abs Error):    {np.mean(np.abs(residuals)):.2f} Kg/ha\n")
            f.write(f"    MBE (Bias):         {np.mean(residuals):.2f} Kg/ha\n")
    ...
    # 5. Generate prediction samples CSV for client inspection
    print("\n[3/3] Generating prediction samples CSV...")
    # Load labels to decode categorical encoders
    with open(os.path.join(PREPROCESSED_DIR, 'encoders.pkl'), 'rb') as f:
        encoders = pickle.load(f)
        
    state_enc = X_test[:, 12].astype(int)
    dist_enc = X_test[:, 13].astype(int)
    crop_enc = X_test[:, 14].astype(int)
    
    # We also need Year and Area back in original scale
    feature_scaler = scalers['feature_scaler']
    # Build complete scaled array to inverse transform
    dummy_feat = X_test[:, :12].copy()
    dummy_orig = feature_scaler.inverse_transform(dummy_feat)
    
    years = np.round(dummy_orig[:, 0]).astype(int)
    areas = np.round(dummy_orig[:, 1], 2) # Area in 1000 ha
    
    states = encoders['State Name'].inverse_transform(state_enc)
    districts = encoders['Dist Name'].inverse_transform(dist_enc)
    crops = encoders['Crop'].inverse_transform(crop_enc)
    ...
    samples_df = pd.DataFrame({
        ...
        'Year': years,
        'Area_1000ha': areas,
        'Actual_Yield_Kg_ha': np.round(y_test_raw, 1)
    })
    
    for name, pred in unscaled_preds.items():
        # Sanitize name for column headers
        col = name.replace('Option A: ', 'A_').replace('Option B: ', 'B_').replace('Option C: ', 'C_').replace(' ', '_').replace(':', '')
        samples_df[col] = np.round(pred, 1)
        
    # Pick a random sample of 100 rows stratified/spread across crops to save
    ...
    # Plot Actual vs Predicted scatter for top 3 models
    ...
    for i, name in enumerate(top_models):
        ...
        ax.set_title(f"{name}\nR²={results_df.iloc[i]['R²']:.4f} | RMSE={results_df.iloc[i]['RMSE']:.1f}", fontweight='bold')
    ...
    # Plot residuals distribution comparison for Option A Stacking, Option B XGB, Option C XGB
    residuals_compare = {
    ...
    ax.set_xlabel('Residual Error (Actual - Predicted, Kg/ha)')
    ...
    # Plot final comparative RMSE and R2 bar chart
    fig, axes = plt.subplots(1, 2, figsize=(18, 8))
    
    # Sort for plotting (best RMSE at the bottom, so bar length is shorter)
    results_sorted_rmse = results_df.sort_values(by='RMSE', ascending=False)
    axes[0].barh(results_sorted_rmse['Model'], results_sorted_rmse['RMSE'], color='skyblue', edgecolor='black')
    axes[0].set_title('Model RMSE (Lower is Better)', fontweight='bold', fontsize=14)
    axes[0].set_xlabel('RMSE (Kg/ha)')
    
    # Sort for plotting R2 (best R2 at the bottom, so bar length is longer)
    results_sorted_r2 = results_df.sort_values(by='R²', ascending=True)
    axes[1].barh(results_sorted_r2['Model'], results_sorted_r2['R²'], color='lightgreen', edgecolor='black')
