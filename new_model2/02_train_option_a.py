...
def build_mlp_model(num_countries, num_crops, num_features, lr=0.001):
    """
    Build MLP Neural Network with Embedding layers for categorical inputs.
    Area (Country) -> 16-dim embedding
    Item (Crop)    ->  8-dim embedding
    """
    # ── Inputs ──
    ...
    # Flatten embeddings
    country_flat = layers.Flatten()(country_emb)
    crop_flat    = layers.Flatten()(crop_emb)

    # ── Concatenate ──
    x = layers.Concatenate()([numeric_input, country_flat, crop_flat])
    ...
    model = Model(inputs=[numeric_input, country_input, crop_input], outputs=output, name='MLP_Embed')
    model.compile(
...
def run_training_option_a(epochs=100, batch_size=256, n_estimators=100, learning_rate=0.001):
    print("="*80)
    print("  CROP YIELD PREDICTION (OLD DATASET) — OPTION A TRAINING")
    ...
    # Feature layout:
    # Indices 0..23  -> numeric (Year + 3 current + 20 lags)
    # Index   24     -> Area_encoded (Country)
    # Index   25     -> Item_encoded (Crop)
    NUM_NUMERIC = X_train.shape[1] - 2   # all columns except the 2 categorical ones

    X_train_num     = X_train[:, :NUM_NUMERIC]
    X_train_country = X_train[:, NUM_NUMERIC:NUM_NUMERIC+1]
    X_train_crop    = X_train[:, NUM_NUMERIC+1:NUM_NUMERIC+2]

    X_val_num     = X_val[:, :NUM_NUMERIC]
    X_val_country = X_val[:, NUM_NUMERIC:NUM_NUMERIC+1]
    X_val_crop    = X_val[:, NUM_NUMERIC+1:NUM_NUMERIC+2]

    X_test_num     = X_test[:, :NUM_NUMERIC]
    X_test_country = X_test[:, NUM_NUMERIC:NUM_NUMERIC+1]
    X_test_crop    = X_test[:, NUM_NUMERIC+1:NUM_NUMERIC+2]

    print(f"    Train: {X_train.shape[0]:,} samples | Val: {X_val.shape[0]:,} samples | Test: {X_test.shape[0]:,} samples")
    print(f"    Numeric features: {NUM_NUMERIC} | Countries: {num_countries} | Crops: {num_crops}")
    ...
    mlp_model = build_mlp_model(num_countries, num_crops, num_features=NUM_NUMERIC, lr=learning_rate)
    ...
    mlp_model.fit(
        x=[X_train_num, X_train_country, X_train_crop],
        y=y_train,
        validation_data=([X_val_num, X_val_country, X_val_crop], y_val),
        epochs=epochs,
    ...
    mlp_val_preds = mlp_model.predict([X_val_num, X_val_country, X_val_crop]).flatten()
    ...
    mlp_test_preds = mlp_model.predict([X_test_num, X_test_country, X_test_crop]).flatten()
